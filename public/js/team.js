$(document).ready(function() {
  ...
  // Getting the initial list of Trainers
  getTrainers();

  // A function to handle what happens when the form is submitted to create a new Trainer
  function handleTrainerFormSubmit(event) {
    ...
    // Calling the upsertTrainer function and passing in the value of the name input
    upsertTrainer({
      name: nameInput
        .val()
        .trim()
  ...
  // A function for creating a Trainer. Calls getTrainers upon completion
  function upsertTrainer(trainerData) {
    $.post("/api/trainers", trainerData)
      .then(getTrainers);
  }

  // Function for creating a new list row for Trainers
  ...
  // Function for retrieving Trainers and getting them ready to be rendered to the page
  ...
  // A function for rendering the list of Trainers to the page
  ...
  // Function for handling what to render when there are no Trainers
  function renderEmpty() {
    var alertDiv = $("<div>");
    alertDiv.addClass("alert alert-danger");
    alertDiv.text("No Trainers have created any teams.  Be the first!");
